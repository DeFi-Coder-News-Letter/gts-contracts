# Gold Trust Standard (GTS)
Gold Trust Standard-issued USD-collateralized ERC20 stablecoin public smart contract repository.

## ABI, Address, and Verification

The contract abi is in `GTS.abi`. It is the abi of the implementation contract.
...
Gold Trust Standard (GTS) is an ERC20 token that is Centrally Minted and Burned by GTS,
representing the trusted party backing the token with USD.

### ERC20 Token

The public interface of GTS Standard is the ERC20 interface
...
The total supply of GTS is backed by fiat held in reserve at Paxos.
...
requests for the purchase and redemption of GTS.

The supply control interface includes methods to get the current address
of the supply controller, and events to monitor the change in supply of GTS.
...
and approvals of the GTS token. The ability to pause is controlled by a single `owner` role,
...
The `assetProtectionRole` can freeze and unfreeze the GTS balance of any address on chain.
...
Freezing is something that Gold Trust Standard will not do on its own accord,
